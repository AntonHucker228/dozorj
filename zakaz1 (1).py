...
class Database:
    ...
    def spend_points(self, user_id: int, amount: int) -> bool:
        """Списывает баллы у пользователя. Возвращает True если успешно."""
        conn = self.get_connection()
        cursor = conn.cursor()
...
def spin_lottery() -> Optional[dict]:
    """Крутим лотерею и возвращаем приз или None (неудача)"""
